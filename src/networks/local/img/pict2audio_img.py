...
img_train = data_processor.load_data(label_train, img_train, imagePath_train, "img_train/")

print('[INFO] loading validation dataset...')
label_validation = []
img_validation = np.empty((0, PICT_WIDTH, PICT_WIDTH, NB_CHANNEL))
img_validation = data_processor.load_data(label_validation, img_validation, imagePath_validation, "img_validation/")
...
[x_train, y_train] = data_processor.shuffle_data(img_train, label_train, NB_TRAIN)
[x_validation, y_validation] = data_processor.shuffle_data(img_validation, label_validation, NB_VALIDATION)
...
y_train_pitch = data_processor.gen_features_tabs(y_train, y_train_pitch, 0)
y_validation_pitch = data_processor.gen_features_tabs(y_validation, y_validation_pitch, 0)

print('y_train_pitch.shape=', y_train_pitch.shape)
print('y_validation_pitch.shape=', y_validation_pitch.shape)

y_train_pitch, y_validation_pitch = network.categorical(y_train_pitch, y_validation_pitch, NB_PITCH)
...
data_processor.gen_features_tabs(y_train, y_train_thick, 1)
data_processor.gen_features_tabs(y_validation, y_validation_thick, 1)
y_train_thick, y_validation_thick = network.categorical(y_train_thick, y_validation_thick, NB_THICK)
...
data_processor.gen_features_tabs(y_train, y_train_color, 2)
data_processor.gen_features_tabs(y_validation, y_validation_color, 2)
y_train_color, y_validation_color = network.categorical(y_train_color, y_validation_color, NB_COLOR)
...
data_generator.generate_pict(NB_VERSION_TESTGEN, 'testgen', 0, [1, 1, 1])
...
img_testgen = data_processor.load_data(label_testgen, img_testgen, imagePath_testgen, "img_testgen/")


print("[INFO] shuffle testgen dataset...")
[x_testgen, y_testgenstat] = data_processor.shuffle_data(img_testgen, label_testgen, NB_TESTGEN)
...
print('pitch ' + str(test_accuracy(y_testgenstat_pitch, y_testgen_pitch)))
print('thick ' + str(test_accuracy(y_testgenstat_thick, y_testgen_thick)))
print('color ' + str(test_accuracy(y_testgenstat_color, y_testgen_color)))
